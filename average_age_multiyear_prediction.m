...
%add header back and join pred column
...
a_preds = new_deNorm(a_pred_2024, a_res);
a_preds = array2table(a_preds);
a_preds = [a_labels a_preds];
a_preds.Properties.VariableNames = a_pred_2024_header;
writetable(a_preds,'avg_wage_multiyear_predictions_updated.txt','Delimiter',' ')%changed txt file with updated deNorm 